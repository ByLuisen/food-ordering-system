...
public class PaymentRequestMessageListenerImpl implements PaymentRequestMessageListener {
    ...
    private final static int MAX_EXECUTION = 100;
    ...
    private void processPayment(Function<PaymentRequest, Boolean> func,
                                PaymentRequest paymentRequest, String methodName) {
        int execution = 1;
        boolean result;
        do {
            log.info("Executing {} operation for {} time for order id {}", methodName, execution,
                    paymentRequest.getOrderId());
            try {
                result = func.apply(paymentRequest);
                execution++;
            } catch (OptimisticLockingFailureException e) {
                log.warn("Caught OptimisticLockingFailureException exception in {} with message: {}! " +
                                "Retrying for order id: {}!",
